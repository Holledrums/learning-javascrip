...
const totalAmount = (arr) => arr.reduce((total, cur) => total + cur.amount, 0);
...
const incrementedNumbers = (arr) => arr.map((number) => number + 1);
console.log(incrementedNumbers(arrayOfNumbers));
...
const filterEvens = (arr) => arr.filter((number) => number % 2 == 0);
...
const filterItems = (arr, search) =>
  arr.filter((name) => name.includes(search));
...
const sum = (arr) => arr.reduce((num, cur) => num + cur, 0);
